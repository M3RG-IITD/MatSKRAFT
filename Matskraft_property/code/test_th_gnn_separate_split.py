...
# # row_col_class_weights = torch.Tensor(compute_class_weight('balanced', classes=[0, 3, 4, 5, 6, 7], y=all_train_row_col_labels)).to(device)
...
#             # check paper for property --> wont work for properties - very low recall
#             if pred_label in [2,4,6,9,10,13,19]:
#                 propert_name = prop_names[pred_label-2]
#                 if not check_paper_for_prop(pii, propert_name):
#                     comp_gid_pred_dict['pred_col_label'][j] = 0
...
#                             if not check_non_controv_unit(prop_name, unit):
                            ...
                            if not prop_flag:
                                ...
                                continue
#                             pred_label = pr_cl[j]
#                             head_flag = check_heading(t_name, r_table, prop_col_annotation, caption, pred_label)
#                             if not head_flag:
#                                 continue
...
#                             if not check_non_controv_unit(prop_name, unit):
                            ...
                            #change the label, delete the respective tuples
                                ...
                                continue
#                             pred_label = pr_rl[i]
#                             head_flag = check_heading(index,r_table ,prop_col_annotation, caption, pred_label)
#                             if not head_flag:
#                                 continue
...
def eval_model(split, debug=False):
    ...
    #only_prop_metrics = pd.DataFrame(classification_report(y_comp_true, y_comp_pred, labels=[2, 3, 4, 5], output_dict=True)).round(3)
    ...
    #pickle.dump(row_col_pred_dict, open(os.path.join(os.getcwd(), split+'_row_col_pred_dict.pkl'), 'wb'))
    # print(f'gold tuples {gold_tuples[split]}')
    #pickle.dump(gold_tuples[split], open(os.path.join(os.getcwd(), split+'_gold_tuples.pkl'), 'wb'))
    #print()
    # print(f'pred tuples {pred_tuples}')
    #pickle.dump(all_pred_tuples, open(os.path.join(os.getcwd(), split+'_pred_tuples.pkl'), 'wb'))
    #print()
    
#     m_true_tuples = temp_cut_off(all_true_tuples)
#     mod_true_tuples = remove_tuples_on_units(m_true_tuples)

    all_pred_tuples_upd = [(a, b, eval_expr(c) if isinstance(c, str) else c, d) for a, b, c, d in all_pred_tuples]
...
#             print(f'tuples outside = {tuples}')
            ...
            t_idx = int(tuples[0].split('_')[1])
            prop_name = tuples[1]
            unit = tuples[-1]
            check_unit = check_non_controv_unit(prop_name, unit)
            if not check_unit:   
               
                new_unit = find_unit_in_paper(pii, t_idx, prop_name)
...
#                     if not check_paper_for_prop(pii, t_idx, prop_name):
#                         #remove tuples form pred_tuples
#                         new_pred_tup.remove(tuples)
#                         continue
                        
                if new_unit=='' and prop_name == "Fracture toughness":
#                     print(f'Old tuples = {tuples}')
                    if (tuples[2]>0.3 and tuples[2]<10) or (tuples[-1].lower().startswith("mp")): #Fracture toughness values for glass on this unit lies in this range
                        new_unit = 'MPam1/2'
                        change = 1
                        new_unit_norm = norm_unit(new_unit, prop_name)
                        new_tuple = (*tuples[:-1], new_unit_norm)
...
#         if change == 1:
#             print(f'Old tuples = {old_tuples}\n')
#             print(f'New pred tuples = {pred_tuples}\n')
#             print(f'True tuples = {true_tuples}\n')
#             print('-------------------------')
            

#             print(prop_res[split]['ret_tuples_pred'][index]) # = pred_tuples
#         prop_res[split]['ret_tuples_pred'][index]  = pred_tuples
    ...
    tuple_prop_metrics = get_property_metrics(ulti_gold_tuples, new_pred_tup, prop_names, split)
    ...
    for key, value in tuple_prop_metrics.items():
        print(f'{key} :: {value}')
#     print(tuple_prop_metrics)
    print()
    # composition_metrics = get_composition_metrics(gold_tuples[split], all_pred_tuples)

    # For saving tuples:
    pickle.dump(new_pred_tup, open(os.path.join(os.getcwd(), split+'_pred_tuples_no_checker.pkl'), 'wb'))
    pickle.dump(ulti_gold_tuples, open(os.path.join(os.getcwd(), split+'_gold_tuples.pkl'), 'wb'))
...
#             violations += v
#             total += t
#         print(f'\t {n} violations: {violations}/{total}')
#         res[f'{n}_violations'] = violations
        
        
# split_arr = ['train', 'val', 'test']
# res_prop = {'identifier':[], 'y_comp_pred':[],'ret_comp_pred':[], 'ret_tuples_pred':[], 'pred_tuples':[], 'gold_tuples':[]}


# for keyy in res_prop.keys():
#     for keys in split_arr:
#         res_prop[keyy] += res[keys][keyy]
end_time = time.time()
elapsed  = end_time - start_time
total_tables = sum(len(res[split]['identifier']) for split in ['val', 'test'] if 'identifier' in res[split])
...
if args.res_file:
    ...
    print(os.path.join(table_dir, 'res_dir_without_threshold'))
    pickle.dump(res, open(os.path.join(table_dir,'res_dir_without_threshold','pegamat_val_test_pred_gold.pkl') , 'wb'))
