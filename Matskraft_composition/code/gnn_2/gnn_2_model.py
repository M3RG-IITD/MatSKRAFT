...
#         config = AutoConfig.from_pretrained('../../../saved_matscibert', cache_dir=args['cache_dir'])
        self.encoder = AutoModel.from_pretrained(args['lm_name'], config=config, cache_dir=args['cache_dir'])
#         self.encoder = AutoModel.from_pretrained('../../../saved_matscibert', config=config, cache_dir=args['cache_dir'])
    ...
    def forward(self, inps):
        ...
        del lm_inp
#         print('Size of emb and cell_h')
#         #print(embs.size()) #list, not tensor
#         print(len(embs)) #variable
#         print(len(embs[0])) #160
#         print(len(embs[0][0])) #768
#         #print(len(embs[0][0][0]))
        cell_h = torch.cat(embs)
#         print(cell_h.size()) #torch.Size([variable, 768])
        del embs
