...
def get_thresholded_labels(labels, softmax_logits, th):
...
#             come here
    ...
    if not debug:
        ...
        (tuple_metrics, ret_tuples_pred, ret_tuples_gold[split]), composition_metrics, type_labels
...
for s in splits[1:]:
    ...
    (res[s]['edge_stats'], res[s]['edge_pred']), (res[s]['tuple_metrics'], res[s]['tuples_pred'], res[s]['tuples_true']), \
...
if args.res_file:
    os.makedirs(os.path.join(table_dir, 'dir_res'), exist_ok=True)
    pickle.dump(res, open(os.path.join(table_dir, 'dir_res', args.res_file), 'wb'))
    # os.remove(args.model_save_file)
print('done_thresholding')