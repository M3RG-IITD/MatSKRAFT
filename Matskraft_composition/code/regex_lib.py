...
#             print(sub_comp_text, sub_pat_1, sub_pat_2)
#             print(f'parsed_sub_comp = {parsed_sub_comp}')
#             print()
...
def x_parse_5(s, sub_pat, pat, specific=False):
    ...
    for m in re.finditer(pat, s):
        ...
        for sub_comp in re.findall(sub_pat_3, comp):
            ...
            if sub_comp_text[0] == '(' and sub_comp_text[-1] == ')':
                assert sub_comp_text[-1] == ')'
                sub_comp_text = sub_comp_text[1:-1].strip()
            elif sub_comp_text[0] == '[' and sub_comp_text[-1] == ']':
                assert sub_comp_text[-1] == ']'
...
def _parse_composition_inner(s, compounds=None):
    ...
    start = r'^[1-9|a-zA-Z][\-]'
    reg1 = start + r'([A-Z][a-z](\d*\.\d+|\d*))*$'
    reg2 = start + r'(\(([A-Z][a-z]?(\d*\.\d+|\d*)?)+\)(\d*\.\d+|\d*))+$'
    reg3a = r'((\d*\.\d+|\d*)?[A-Z][a-z]?(\d*\.\d+|\d*)?)+'
    reg3 = start + r'((' + reg3a + r'\s*\*\s*)+' + reg3a + r'$)'
    
    if re.match(reg1, s) or re.match(reg2, s) or re.match(reg3, s):
        s = re.sub(start, '', s)
